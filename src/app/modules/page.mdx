...
Modules are an essential part of Protocol — they are the containers for the messages between you, your
contacts, and infrastructure. On this page, we’ll dive into the different conversation endpoints you can use
to manage conversations programmatically. We'll look at how to query, create, update, and delete
conversations. {{ className: 'lead' }}

## The conversation model

The conversation model contains all the information about the conversations between you and your contacts. In
addition, conversations can also be group-based with more than one contact, they can have a pinned message,
and they can be muted.
...
## List all conversations {{ tag: 'GET', label: '/v1/modules' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="GET" label="/v1/modules">
...
## Create a conversation {{ tag: 'POST', label: '/v1/modules' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/v1/modules">

    ```bash {{ title: 'cURL' }}
    curl https://api.protocol.chat/v1/modules \
      -H "Authorization: Bearer {token}" \
...
## Retrieve a conversation {{ tag: 'GET', label: '/v1/modules/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="GET" label="/v1/modules/xgQQXg3hrtjh7AvZ">

    ```bash {{ title: 'cURL' }}
    curl https://api.protocol.chat/v1/modules/xgQQXg3hrtjh7AvZ \
      -H "Authorization: Bearer {token}"
    ```
...
## Update a conversation {{ tag: 'PUT', label: '/v1/modules/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="PUT" label="/v1/modules/xgQQXg3hrtjh7AvZ">

    ```bash {{ title: 'cURL' }}
    curl -X PUT https://api.protocol.chat/v1/modules/xgQQXg3hrtjh7AvZ \
      -H "Authorization: Bearer {token}" \
      -d 'is_muted'=true
    ```
...
## Delete a conversation {{ tag: 'DELETE', label: '/v1/modules/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="DELETE" label="/v1/modules/xgQQXg3hrtjh7AvZ">

    ```bash {{ title: 'cURL' }}
    curl -X DELETE https://api.protocol.chat/v1/modules/xgQQXg3hrtjh7AvZ \
      -H "Authorization: Bearer {token}"
    ```
